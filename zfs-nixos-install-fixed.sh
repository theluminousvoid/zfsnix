...
echo -e "${GREEN}=== ZFS on Root Installation Script (with zvol swap) ===${NC}"
...

{
  ...
  # ZFS kernel params - оптимизированные для zvol
  boot.kernelParams = [ 
  ...
  # Специальные sysctl для предотвращения deadlock со swap на zvol
  boot.kernel.sysctl = {
    "vm.swappiness" = 10;                # Меньше использовать swap
    "vm.min_free_kbytes" = 131072;       # 128MB минимум свободной памяти
...
echo "  ✓ Added zvol-specific kernel parameters and sysctl settings"
...
echo "  ✓ rpool/swap -> zvol for swap (16GB, zle compression)"
echo ""
echo -e "${GREEN}Swap zvol advantages:${NC}"
echo "  ✓ Encrypted through ZFS native encryption"
echo "  ✓ Compressed with zle (zero-length encoding)"
echo "  ✓ Part of ZFS snapshots (excluded from auto-snapshot)"
echo "  ✓ TRIM support through ZFS"
echo "  ✓ Unified storage management"
echo ""
